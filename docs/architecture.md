...
### The graph dbt actually builds

Every arrow below is a `ref()` recorded in `dbt/target/manifest.json`, never a
drawing. `scripts/generate_dbt_dag.py --inject docs/architecture.md` rewrites the
block, and `--check` exits 1 when it no longer matches the project, so a model
added without regenerating shows up as a diff rather than as a diagram that is
quietly out of date.

Two foldings, neither of which makes an arrow less exact: the 10 `raw` tables are
in section 1 and are not repeated here, and the 7 seeds are one box — every arrow
leaving it is a real dependency of the model it reaches. Staging models are
**not** folded per publisher: that would halve the picture while claiming
`fact_affordability` depends on all four Statistics Canada models when it depends
on two.

The seed box is labelled with how many seeds the **drawn models** read, which is
six, not the seven the repository holds. The seventh,
`apciq_known_publisher_defect`, is read by no model at all — only by three
singular tests, including `assert_apciq_controls_hold_or_are_accounted_for`. It
is a declaration of a known publisher defect, not a source of data, so a lineage
of models is right to leave it out.

<!-- dbt-dag:begin -- generated by scripts/generate_dbt_dag.py, do not edit by hand -->

```mermaid
flowchart LR
  subgraph STG_apciq["APCIQ"]
    stg_apciq__barometer_statistics["barometer_statistics"]
    stg_apciq__control_totals["control_totals"]
  end
  subgraph STG_bank_of_canada["Bank of Canada"]
    stg_bank_of_canada__interest_rates["interest_rates"]
  end
  subgraph STG_statcan["Statistics Canada"]
    stg_statcan__census_tracts["census_tracts"]
    stg_statcan__consumer_price_index["consumer_price_index"]
    stg_statcan__dissemination_areas["dissemination_areas"]
    stg_statcan__household_income["household_income"]
  end
  subgraph STG_montreal_open_data["Donnees Montreal"]
    stg_montreal_open_data__administrative_boundaries["administrative_boundaries"]
    stg_montreal_open_data__neighbourhoods["neighbourhoods"]
  end
  subgraph INT["intermediate"]
    int_geography__dissemination_area_place["dissemination_area_place"]
    int_statcan__cpi_quarterly_index["cpi_quarterly_index"]
  end
  SEEDS[/"6 seeds<br/>published parameters"/]
  subgraph MARTS["marts"]
    bridge_apciq_sector_geography["bridge_apciq_sector_geography"]
    bridge_census_tract_admin_place["bridge_census_tract_admin_place"]
    bridge_census_tract_apciq_sector["bridge_census_tract_apciq_sector"]
    dim_geography["dim_geography"]
    dim_household_profile["dim_household_profile"]
    fact_affordability["fact_affordability"]
    fact_interest_rate["fact_interest_rate"]
    fact_market["fact_market"]
    fact_market_trailing_12m["fact_market_trailing_12m"]
    fact_mortgage_scenario["fact_mortgage_scenario"]
    map_place["map_place"]
  end
  DECLARED["3 declared dimensions<br/>dim_date<br/>dim_interest_rate_series<br/>dim_property_type<br/>no dbt parent, read by Power BI"]
  stg_bank_of_canada__interest_rates --> fact_interest_rate
  stg_statcan__census_tracts --> stg_statcan__household_income
  stg_statcan__dissemination_areas --> int_geography__dissemination_area_place
  stg_montreal_open_data__administrative_boundaries --> int_geography__dissemination_area_place
  SEEDS --> int_geography__dissemination_area_place
  stg_statcan__consumer_price_index --> int_statcan__cpi_quarterly_index
  stg_montreal_open_data__administrative_boundaries --> dim_geography
  SEEDS --> dim_geography
  bridge_census_tract_apciq_sector --> dim_geography
  stg_statcan__census_tracts --> dim_geography
  bridge_census_tract_admin_place --> dim_geography
  SEEDS --> bridge_apciq_sector_geography
  stg_montreal_open_data__administrative_boundaries --> bridge_apciq_sector_geography
  int_geography__dissemination_area_place --> bridge_census_tract_admin_place
  bridge_apciq_sector_geography --> bridge_census_tract_admin_place
  int_geography__dissemination_area_place --> bridge_census_tract_apciq_sector
  bridge_apciq_sector_geography --> bridge_census_tract_apciq_sector
  stg_montreal_open_data__neighbourhoods --> bridge_census_tract_apciq_sector
  SEEDS --> bridge_census_tract_apciq_sector
  bridge_census_tract_apciq_sector --> fact_affordability
  fact_mortgage_scenario --> fact_affordability
  dim_household_profile --> fact_affordability
  stg_statcan__household_income --> fact_affordability
  int_statcan__cpi_quarterly_index --> fact_affordability
  stg_apciq__barometer_statistics --> fact_market
  stg_apciq__control_totals --> fact_market
  SEEDS --> fact_mortgage_scenario
  fact_market --> fact_mortgage_scenario
  stg_bank_of_canada__interest_rates --> fact_mortgage_scenario
  stg_apciq__barometer_statistics --> fact_market_trailing_12m
  stg_apciq__control_totals --> fact_market_trailing_12m
  dim_geography --> map_place
  bridge_apciq_sector_geography --> map_place
  SEEDS --> map_place
  stg_montreal_open_data__neighbourhoods --> map_place
```
